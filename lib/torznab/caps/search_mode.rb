...
module Torznab
  class Caps
    ...
    class SearchMode
      ...
      def type=(type)
        raise Torznab::Errors::CapsError, 'Type must be a valid caps search mode' unless type.include? 'search'
        @type = type
      end

      def available=(available)
        if !available.is_a?(TrueClass) && !available.is_a?(FalseClass)
          raise Torznab::Errors::CapsError, 'Available must be a boolean'
        end
      ...
      def validate_supported_params(supported_params, default_params)
        ...
        unless supported_params.is_a? Array
          raise Torznab::Errors::CapsError, 'Supported params must either be an array or nil'
        end
