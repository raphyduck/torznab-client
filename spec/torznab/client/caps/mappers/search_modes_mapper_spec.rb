...
describe Torznab::Client::Caps::Mappers::SearchModesMapper do
  describe '.map' do
    before do
      ...
      allow(Torznab::Client::Caps::Mappers::SearchModeMapper).to receive(:map).and_return search_mode
    end
    let(:search_modes) do
    ...
    context 'when the search node is missing from the searching node' do
      ...
      it { expect { subject }.to raise_error Torznab::Client::Errors::XmlError, 'Search mode with name search must exist' }
    end

    context 'when the search node is missing from the searching node' do
      let(:without_tv_search) { true }
      it { expect { subject }.to raise_error Torznab::Client::Errors::XmlError, 'Search mode with name tv-search must exist' }
    end

    context 'when the search node is missing from the searching node' do
      let(:without_movie_search) { true }
      it { expect { subject }.to raise_error Torznab::Client::Errors::XmlError, 'Search mode with name movie-search must exist' }
    end

    context 'when all the required search nodes are in the searching node', :end_with_subject do
      it { expect(search_modes).to receive(:search=).with instance_of Torznab::Client::Caps::SearchMode }
      it { expect(search_modes).to receive(:tv_search=).with instance_of Torznab::Client::Caps::SearchMode }
      it { expect(search_modes).to receive(:movie_search=).with instance_of Torznab::Client::Caps::SearchMode }
    end
  end
end
